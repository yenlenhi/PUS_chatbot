            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for chunk in chunks:
                    cursor.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        chunk.content, chunk.source_file, chunk.page_number, chunk.chunk_index,
                        chunk.heading_text, chunk.heading_level, chunk.heading_number, chunk.parent_heading,
                        chunk.is_sub_chunk, chunk.sub_chunk_index, chunk.total_sub_chunks, chunk.chunk_type,
    ...
    def get_all_chunks(self) -> List[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('''
                    SELECT id, content, source_file, page_number, chunk_index, created_at
                    FROM chunks
                    ORDER BY id
                ''')
