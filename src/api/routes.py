...
async def chat_endpoint(request: ChatRequest, rag: RAGService = Depends(get_rag_service)):
    ...
    try:
        ...
        response = ChatResponse(
            answer=rag_response.get('answer', "Xin lỗi, tôi không thể trả lời câu hỏi này."),
            sources=rag_response.get('sources', []),
            confidence=rag_response.get('confidence', 0.0),
            conversation_id=rag_response.get('conversation_id', request.conversation_id or "default"),
...
async def search_endpoint(request: SearchRequest, rag: RAGService = Depends(get_rag_service)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Đã xảy ra lỗi khi tìm kiếm. Vui lòng thử lại sau."
        )
...
async def health_endpoint(rag: RAGService = Depends(get_rag_service)):
    ...
    try:
        ...
        database_status = health['components'].get('database', {}).get('status', 'unknown')
...
async def get_conversation_endpoint(conversation_id: str, rag: RAGService = Depends(get_rag_service)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Đã xảy ra lỗi khi lấy lịch sử hội thoại."
        )
...
async def stats_endpoint(rag: RAGService = Depends(get_rag_service)):
    ...
    try:
        ...
        # Get FAISS stats
        faiss_stats = rag.faiss_service.get_index_stats()
        ...
        raise HTTPException(
            status_code=500,
            detail="Đã xảy ra lỗi khi lấy thống kê hệ thống."
        )
