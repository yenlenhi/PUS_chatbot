...
class HeadingChunker:
    ...
    def __init__(
        ...
        Args:
            min_chunk_size: Minimum size of a chunk in characters. Chunks smaller than this will be merged.
    ...
    def chunk_by_headings(
        self, text: str, source_file: str, page_number: Optional[int] = None
    ) -> List[DocumentChunk]:
        """
    ...
    def _chunk_without_headings(
        self, text: str, source_file: str, page_number: Optional[int]
    ) -> List[DocumentChunk]:
    ...
    def _process_section(
        ...
        log.info(
            f"Section '{heading.get('text', 'Unknown')}' is large ({char_count} chars), splitting..."
        )

        # Try to find sub-headings within this section
        sub_chunks = self._split_large_section(
            section_text, heading, source_file, page_number, start_chunk_index
        )
    ...
    def _split_by_paragraphs(
        ...
        # Split by double newlines (paragraphs), preserving them for rejoining
        ...
        for i in range(0, len(paragraphs), 2):
            ...
            # Determine the separator (usually '\n\n')
            separator = paragraphs[i + 1] if i + 1 < len(paragraphs) else ""

            # If current_chunk is empty, start with the paragraph
            ...
            if (
                len(current_chunk_content) + len(separator) + len(paragraph)
                > self.max_chunk_size
            ):
                chunks.append(
                    DocumentChunk(
                        ...
                        chunk_type="heading",
                        word_count=len(current_chunk_content.split()),
                        char_count=len(current_chunk_content),
                    )
                )
                chunk_index += 1
                sub_chunk_index += 1
                current_chunk_content = paragraph  # Start a new chunk
            else:
                # Otherwise, add the paragraph to the current chunk
                current_chunk_content += separator + paragraph
        ...
        # Update total_sub_chunks for all created sub-chunks
        total_sub_chunks = len(chunks)
        for chunk in chunks:
            chunk.total_sub_chunks = total_sub_chunks
    ...
    def analyze_chunks(self, chunks: List[DocumentChunk]) -> Dict[str, Any]:
        ...
        word_counts = [
            chunk.word_count or len(chunk.content.split()) for chunk in chunks
        ]
        ...
        for chunk in chunks:
            ...
            analysis["chunks_by_type"][chunk_type] = (
                analysis["chunks_by_type"].get(chunk_type, 0) + 1
            )

            if chunk.heading_level:
                level = chunk.heading_level
                analysis["chunks_by_heading_level"][level] = (
                    analysis["chunks_by_heading_level"].get(level, 0) + 1
                )
    ...
    def _merge_small_chunks(self, chunks: List[DocumentChunk]) -> List[DocumentChunk]:
        ...
        This implementation attempts to merge with the previous or next chunk.
        """
        if not chunks:
            return []

        # Use a mutable list to handle merges
        ...
        while i < len(processed_chunks):
            ...
            # Chunk is small, try to merge it
            # Option 1: Try to merge with the previous chunk
            if i > 0:
                prev_chunk = processed_chunks[i - 1]
                prev_size = prev_chunk.char_count or len(prev_chunk.content)
                combined_size = prev_size + current_size

                if combined_size <= self.max_chunk_size and self._can_merge_chunks(
                    prev_chunk, current_chunk
                ):
                    # Merge current into previous
                    merged_content = prev_chunk.content + "\n\n" + current_chunk.content
                    prev_chunk.content = merged_content
                    prev_chunk.char_count = len(merged_content)
                    prev_chunk.word_count = len(merged_content.split())

                    # Remove current chunk and stay at the same index `i` to re-evaluate the merged chunk
                    processed_chunks.pop(i)
                    continue

            # Option 2: Try to merge with the next chunk
            if i < len(processed_chunks) - 1:
                ...
                if combined_size <= self.max_chunk_size and self._can_merge_chunks(
                    ...
                    current_chunk.char_count = len(merged_content)
                    current_chunk.word_count = len(merged_content.split())

                    # Adopt metadata from the more significant (next) chunk
                    current_chunk.heading_text = next_chunk.heading_text
                    current_chunk.heading_level = next_chunk.heading_level
                    current_chunk.heading_number = next_chunk.heading_number

                    # Remove next chunk
                    processed_chunks.pop(i + 1)
                    # Stay at the same index `i` to re-evaluate the merged chunk
                    continue

            # If no merge was possible, move to the next chunk
            i += 1

        # Update chunk indices after all merges are done
    ...
    def _can_merge_chunks(self, chunk1: DocumentChunk, chunk2: DocumentChunk) -> bool:
        ...
        if (
            chunk1.page_number
            and chunk2.page_number
            and chunk1.page_number != chunk2.page_number
        ):
            return False
        ...
        if chunk1.heading_number and chunk2.heading_number:
            ...
            if (
                chunk1.parent_heading == chunk2.heading_number
                or chunk2.parent_heading == chunk1.heading_number
            ):
                return True
        ...
        if (chunk1.heading_text and not chunk2.heading_text) or (
            chunk2.heading_text and not chunk1.heading_text
        ):
            return True
